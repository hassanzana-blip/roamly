...
| 7 | Passenger totals and NOK FX provenance | partial | `priceMode: total`; Norges Bank rate and date shown; "approx." marked. Gap (follow-up, documented; not needed for the request-match fix): the response's `priceMode` metadata is not checked. The app never multiplies an ambiguous price. |
