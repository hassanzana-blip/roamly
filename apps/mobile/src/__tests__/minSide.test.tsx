import { AccessibilityInfo, StyleSheet, Text as RNText, type StyleProp, type TextStyle, type ViewStyle } from "react-native";
...
function seed({ recent, saved, homeAirport }: { recent?: unknown[]; saved?: unknown[]; homeAirport?: unknown }) {
...
async function renderMinSide({ signedIn = false, locale = "nb", routes = {} }: { signedIn?: boolean; locale?: "nb" | "en"; routes?: Parameters<typeof fakeServer>[0] } = {}) {
...
describe("Min side med søk, reisemål og vanlig avreiseflyplass", () => {
  ...
  it("«Reisevaner»: vanlig avreiseflyplass med verdien, og raden åpner flyplassøket for den", async () => {
    ...
    expect(group.getByText("Lagres bare på denne telefonen. Nye søk starter derfra.")).toBeOnTheScreen();
    const row = screen.getByTestId("home-airport-row");
  ...
  it("feilen gjelder søket slik det var: endres skjemaet et annet sted (som på Hjem), står den ikke lenger", async () => {
    ...
    expect(screen.getByTestId("probe")).toHaveTextContent(/^roundtrip BGO→TOS /);
    expect(screen.queryByTestId("hub-error")).toBeNull();
    // Nå går søket.
    await fireEvent.press(screen.getByTestId("hub-saved-tromso"));
    expect(router.push).toHaveBeenCalledWith("/resultater");
...
describe("Min side innlogget", () => {
  ...
  it("«På hellosky.no»: kundens sider som lenker med hint, åpnet i Safari-visning", async () => {
    ...
    for (const [id, title, url] of [
      ["link-trips", "Mine reiser", WEB_PAGES.trips],
      ...
      expect(link).toHaveProp("accessibilityLabel", title);
  ...
  it("engelsk: sidene på hellosky.no er på norsk, og det sies", async () => {
    ...
    expect(screen.getByTestId("link-trips")).toHaveProp("accessibilityLabel", "My trips");
  });
...
describe("Min side i «Cloud + Graphite»", () => {
  ...
  it("grafittøya går opp under statuslinjen, med runde hjørner nederst; siden under er lys", async () => {
    ...
    expect(colorOf(within(screen.getByTestId("preferences-group")).getByText("Lagres bare på denne telefonen. Nye søk starter derfra."))).toBe(colors.textSecondary);
...
describe("flyplassøket fra Min side (vanlig avreiseflyplass)", () => {
  async function renderPicker(params: Record<string, string>) {
    setParams(params);
  ...
  it("med hjem=1: egen tittel og spørsmål, ingen bryter – forklaringen står; valget huskes, blir «Fra» og lukker", async () => {
    ...
    expect(screen.getByTestId("home-airport-note")).toHaveTextContent("Lagres bare på denne telefonen. Nye søk starter derfra.");
