...
function Barcode({ seed }: { seed: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${x} 44`} className="h-11 w-full text-night" aria-hidden="true">
      {bars.map((b, i) => (
...
export default function BoardingPass({
  ...
  return (
    <div className="overflow-hidden rounded-[26px] bg-[#f7f4ec] text-night shadow-2xl shadow-night/40">
      <div className="grid sm:grid-cols-[1.65fr_1fr]">
        {/* Hoveddel */}
        <div className="relative p-6 sm:p-8">
          <div className="flex items-center justify-between">
            <p className="font-mono-label text-[10px] text-night/55">hellosky ✦ boarding pass</p>
            <span className="font-mono-label rounded-full bg-night px-2.5 py-1 text-[9px] text-white">
              {isCouple ? "vi to" : "oppdag"}
            </span>
          </div>

          <p className="font-display mt-6 text-5xl leading-[0.95] sm:text-6xl">{d.city}</p>
          <p className="font-mono-label mt-2 text-[10px] text-night/55">
            {d.country} · {d.tagline}
          </p>
          ...
          <div className="mt-7 flex items-center gap-5">
            <div>
              ...
              <p className="font-mono-label mt-1 text-[9px] text-night/50">Oslo</p>
            </div>
            <svg viewBox="0 0 120 12" className="h-3 flex-1 text-night/45" aria-hidden="true">
            ...
            <div className="text-right">
              ...
              <p className="font-mono-label mt-1 text-[9px] text-night/50">{d.city}</p>
            </div>
          </div>

          <div className="mt-7 grid grid-cols-3 gap-3 border-t border-dashed border-night/20 pt-5">
            <div>
              <p className="font-mono-label text-[9px] text-night/45">Reisende</p>
              <p className="mt-1 truncate text-sm font-bold">{passengerLine}</p>
            </div>
            <div>
              <p className="font-mono-label text-[9px] text-night/45">Dato</p>
              <p className="mt-1 text-sm font-bold">{formatPassDate(date)}</p>
            </div>
            <div>
              <p className="font-mono-label text-[9px] text-night/45">Flight</p>
              <p className="mt-1 text-sm font-bold">HS {d.iata.charCodeAt(0) + d.iata.charCodeAt(2)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative border-t-2 border-dashed border-night/25 bg-[#fdfaf3] p-6 sm:border-l-2 sm:border-t-0">
          <p className="font-mono-label text-[9px] text-night/45">Sete</p>
          <p className="font-display mt-1 text-2xl">{isCouple ? "2A · 2B" : "1A"}</p>
          <p className="font-mono-label mt-4 text-[9px] text-night/45">Vindu, selvfølgelig</p>
          <div className="mt-5">
            <Barcode seed={d.iata + d.city} />
          </div>
          <p className="font-mono-label mt-3 text-[8px] leading-relaxed text-night/40">
