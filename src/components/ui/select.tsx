...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-full items-center justify-between gap-2 whitespace-nowrap rounded-lg border border-input bg-card px-3.5 text-left text-base text-foreground shadow-xs outline-none",
        "transition-[border-color,box-shadow] duration-fast ease-out",
        "hover:border-foreground/40 focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/25",
        "data-[placeholder]:text-muted-foreground/70 aria-invalid:border-destructive disabled:cursor-not-allowed disabled:opacity-50",
        size === "default" ? "h-11" : "h-9 text-sm",
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          "relative z-50 max-h-[min(24rem,var(--radix-select-content-available-height))] min-w-[8rem] overflow-hidden rounded-lg border border-border bg-popover text-popover-foreground shadow-lift",
...
function SelectItem({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Item>) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex min-h-10 w-full cursor-pointer select-none items-center gap-2 rounded-md py-2 pl-3 pr-9 text-sm outline-none",
