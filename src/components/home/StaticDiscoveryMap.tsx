...
/**
 * The self-contained fallback for the discovery map: sand-coloured land on a
 * pale sea, one price pin per destination in view. Land comes from Natural
 * Earth (world-atlas, public domain) and is drawn as plain SVG, so it needs no
 * tile provider and no network. DiscoveryMap (MapLibre) uses it when the tile
 * style cannot be fetched. Pins are HTML buttons laid over the SVG, so they
 * are real, focusable controls with the live price as their name.
 *
...
export type Pin = { id: string; label: string; price: string | null; lat: number; lng: number };
...
export default function StaticDiscoveryMap({
  pins,
  selected,
  onSelect,
  className,
