      ...
      <main id="main" tabIndex={-1} className="mx-auto w-full max-w-6xl px-4 pb-20 pt-28 outline-none sm:px-6">
        <div className="bg-muted/40 border-b border-border -mx-4 -mt-28 mb-10 px-4 pb-12 pt-32 sm:-mx-6 sm:px-6">
          ...
          <h1 className="mt-2 font-display text-4xl sm:text-6xl">
            {t("sp.title1")} <span className="text-skyline">{t("sp.title2")}</span>
          </h1>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <a key={c.title} href={c.href} className="card-lift block rounded-3xl border border-border bg-white p-5 shadow-soft">
        ...
        <section className="mt-16" aria-label={t("sp.team.aria")}>
          <p className="font-mono-label text-[11px] text-skyline">{t("sp.team.kicker")}</p>
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-5 flex w-full items-center justify-center gap-2.5 rounded-full bg-[#25D366] px-6 py-3.5 text-sm font-bold text-night transition-all hover:brightness-105 active:scale-[0.99]"
        ...
        <div className="grid items-start gap-10 lg:grid-cols-[1fr_1fr]">
          ...
          <section>
            ...
            {sent ? (
              <div className="fade-up mt-6 rounded-3xl border border-border bg-muted p-8 text-center">
                <CheckCircle2 className="mx-auto h-10 w-10 text-skyline" />
                <h3 className="mt-4 font-display text-2xl">{t("sp.thanks")}</h3>
                <p className="mt-2 text-sm text-muted-foreground">
                  {t("sp.thanks.a")} <span className="font-bold text-skyline">{sent.ref}</span>
              ...
              >
                ...
                <button
                  ...
                  className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-primary px-6 py-4 text-base font-bold text-primary-foreground transition-all hover:brightness-110 disabled:opacity-40"
                >
          ...
          <section>
            ...
            {!customer && (
              <div className="mt-4 rounded-2xl border hairline bg-card p-5 text-sm text-muted-foreground">
                <p>{t("sp.cases.login")}</p>
                <Link to="/logg-inn?next=/hjelp" className="mt-3 inline-flex min-h-11 items-center rounded-full bg-night px-5 text-sm font-bold text-white">
                  {t("common.login")}
                </Link>
              </div>
            )}
            {customer && casesErrCode === "EMAIL_NOT_VERIFIED" && (
              <div className="mt-4 flex items-start gap-3 rounded-2xl border border-amber-300/60 bg-amber-50 p-4 text-sm">
                <MailWarning className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" aria-hidden="true" />
                <div>
                  <p className="font-semibold text-amber-900">{t("sp.cases.verify")}</p>
                  <button
                    type="button"
            ...
            {cases.data && (
              <div className="mt-4 space-y-3">
                {cases.data.length === 0 && (
                  <p className="rounded-2xl border hairline bg-card p-4 text-sm text-muted-foreground">{t("sp.cases.none", { email: customer?.email ?? "" })}</p>
                )}
                {cases.data.map((c) => (
                  <article key={c.id} className="rounded-2xl border hairline bg-card p-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <span className="font-mono text-sm font-bold text-skyline">{c.reference}</span>
